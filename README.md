...
録音前に「授業資料を添付…」でスライド等(PDF/Word/txt)を選ぶと、**文字起こし前に資料から固有名詞・専門用語を自動抽出して認識ヒントに使い**、要約時にも用語・構成の参考に使われる。
...
# 授業資料(PDF/Word/txt)を参考に渡す（固有名詞の自動抽出＋要約の用語・構成の精度向上）
...
- **幻聴ループ対策**: 無音・低音質区間で同じフレーズが延々と繰り返される Whisper の症状を抑えるため、`condition_on_previous_text=False`（直前出力を次のプロンプトにしない）と無音区間スキップ（`hallucination_silence_threshold=2`）を有効にしている。それでも残る反復は整形時に間引く（`--max-repeats`、途中に空行が挟まっていても1つの塊として扱う）
- **語彙バイアス**: 授業資料を渡すと、文字起こしの前に資料から固有名詞・専門用語を自動抽出し、認識ヒント（`--initial-prompt`）として渡す。抽出には要約と同じエンジンを使う（`claude` の場合はクラウド送信の同意が前提。GUI は同意済みのときのみ実行）。抽出に失敗しても文字起こしは止まらず、手動設定の語彙だけで続行する
- 設定の「固有名詞リスト」（`--vocab` / `vocabulary`）に入れた語は自動抽出より優先される
...
| `--vocab <用語,...>` | 固有名詞リスト（自動抽出より優先。文字起こしと要約の両方で使う） |
| `--material <パス>` | 授業資料（複数可。固有名詞の自動抽出＋要約の参考に使う） |
